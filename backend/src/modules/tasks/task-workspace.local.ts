...
import type { TaskWorkspaceChangeKind, TaskGitStatusWorkingTree } from '../../types/task';
import type {
  TaskWorkspaceCommit,
  TaskWorkspaceFile,
  TaskWorkspaceFileSection,
...
export class LocalTaskWorkspaceError extends Error {
  constructor(
    readonly code: string,
    message: string
  ) {
    super(message);
    this.name = 'LocalTaskWorkspaceError';
  }
}
...
const normalizeRepoRelativePath = (value: string): string => {
  ...
  if (!normalized || normalized === '.' || path.posix.isAbsolute(normalized)) {
    throw new LocalTaskWorkspaceError('INVALID_PATH', 'Invalid workspace path');
  }
...
const ensureWorkspaceExists = async (repoDir: string): Promise<void> => {
  try {
    ...
    throw new LocalTaskWorkspaceError('WORKSPACE_MISSING', 'Task workspace is missing');
  }
  try {
    await access(path.join(repoDir, '.git'));
  } catch {
    throw new LocalTaskWorkspaceError('WORKSPACE_NOT_GIT', 'Task workspace is not a git repository');
  }
};
...
const buildWorkingTree = (sectionSets: {
  staged: Set<string>;
  unstaged: Set<string>;
  untracked: Set<string>;
...
const buildSummary = (workingTree: TaskGitStatusWorkingTree, files: TaskWorkspaceFile[]): TaskWorkspaceSummary => {
  let additions = 0;
  let deletions = 0;
...
export const collectLocalTaskWorkspace = async (repoDir: string): Promise<TaskWorkspaceState> => {
  ...
  const workingTree = buildWorkingTree(sectionSets);
  return {
    source: 'backend',
    live: true,
    readOnly: false,
    ...
    behind,
    workingTree,
    summary: buildSummary(workingTree, files),
    files,
...
const requireGitSuccess = (result: GitResult, message: string): void => {
  ...
  throw new LocalTaskWorkspaceError('GIT_COMMAND_FAILED', `${message}: ${result.stderr.trim() || result.stdout.trim() || 'command failed'}`);
};

const commitWorkspace = async (repoDir: string, message: string): Promise<TaskWorkspaceCommit> => {
  const trimmed = String(message ?? '').trim();
  if (!trimmed) {
    throw new LocalTaskWorkspaceError('COMMIT_MESSAGE_REQUIRED', 'Commit message is required');
  }
...
export const executeLocalTaskWorkspaceOperation = async (params: {
  repoDir: string;
  action: TaskWorkspaceOperation;
  paths?: string[];
  message?: string;
}): Promise<TaskWorkspaceOperationResult> => {
  const workspace = await collectLocalTaskWorkspace(params.repoDir);
  ...
  if (params.action === 'commit') {
    if (!workspace.canCommit) {
      throw new LocalTaskWorkspaceError('NO_STAGED_CHANGES', 'There are no staged changes to commit');
    }
    const commit = await commitWorkspace(params.repoDir, params.message ?? '');
    return {
      commit,
      workspace: await collectLocalTaskWorkspace(params.repoDir)
    };
  }
  ...
  if (params.action === 'stage') {
    requireGitSuccess(await runGit(params.repoDir, ['add', '--', ...targetPaths]), 'Failed to stage changes');
  }

  if (params.action === 'unstage') {
    requireGitSuccess(await runGit(params.repoDir, ['reset', 'HEAD', '--', ...targetPaths]), 'Failed to unstage changes');
  }

  if (params.action === 'discard') {
    requireGitSuccess(
      await runGit(params.repoDir, ['restore', '--source=HEAD', '--staged', '--worktree', '--', ...targetPaths]),
      'Failed to discard changes'
    );
  }

  if (params.action === 'delete_untracked') {
    requireGitSuccess(await runGit(params.repoDir, ['clean', '-fd', '--', ...targetPaths]), 'Failed to delete untracked files');
  }

  return { workspace: await collectLocalTaskWorkspace(params.repoDir) };
};
