...
// Fetch preview status to discover instance names and availability. docs/en/developer/plans/3ldcl6h5d61xj2hsu6as/task_plan.md 3ldcl6h5d61xj2hsu6as
const HELP_TEXT = `Hookcode Preview Status

Usage:
  node scripts/preview_status.mjs --task-group <id>
...
async function main() {
  ...
  const url = buildUrl(baseUrl, `/api/task-groups/${taskGroupId}/preview/status`);
  const headers = buildAuthHeaders(pat);
  await sendRequest({ url, method: 'GET', headers, dryRun: Boolean(args['dry-run']) });
}
