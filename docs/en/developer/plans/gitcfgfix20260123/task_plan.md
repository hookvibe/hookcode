# Task Plan: Fix agent git config key names for push workflow
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. gitcfgfix20260123 */}
...
Identify why agent git config setup uses invalid keys, assess impact on push workflow, and implement a compliant fix with tests and documentation updates.
...
- [x] Define technical approach and fallback behavior
...
- [x] Execute the plan step by step (agent config + readers)
...
- [x] Verify all requirements met (config write + read + push)
...
1. Where are git config keys written and read in the agent workflow? (Agent pre-push guard + config setup in `backend/src/agent/agent.ts`.)
2. What fallback should be used when custom config keys are missing or invalid? (Guard exits early when keys are missing; valid keys now always write.)

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Store git config key names in `backend/src/utils/gitWorkflow.ts` and reuse in agent + tests. | Keeps key names centralized and testable without exposing agent internals. |
