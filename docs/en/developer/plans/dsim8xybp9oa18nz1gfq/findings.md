# Findings & Decisions: Migrate docs.json to Legacy Docs Site
...
{/* Replace legacy docs wording with framework-agnostic phrasing. docs/en/developer/plans/mintlify-docs-20260301/task_plan.md mintlify-docs-20260301 */}
- Replace Mintlify docs tooling with a legacy docs site.
...
{/* Replace legacy docs wording with framework-agnostic phrasing. docs/en/developer/plans/mintlify-docs-20260301/task_plan.md mintlify-docs-20260301 */}
- `docs/docs.json` was a Mintlify docs configuration file and `package.json#dev:doc` used `npx mint dev`; after migration it is replaced by a legacy docs site under `docs/`.
- Existing documentation lives under `docs/en/**` (User Docs, API Reference, Developer Docs, Change Log).
- The planning-with-files skill scripts were updated to skip `docs.json` sync when `docs/docs.json` is absent (legacy docs does not require explicit nav sync).
...
|----------|-----------|
{/* Replace legacy docs wording with framework-agnostic phrasing. docs/en/developer/plans/mintlify-docs-20260301/task_plan.md mintlify-docs-20260301 */}
| Host the legacy docs site in `docs/` as a workspace package | Keeps docs-related tooling co-located with content and matches the existing `cd docs` workflow |
| Point the docs plugin path at `docs/en` (not a nested `docs/docs`) | Avoids moving existing content and keeps URLs predictable |
| Keep docs served under `/docs/*` (legacy docs default) | Avoids needing a special `homePageId` and keeps a clear separation between site home and docs |
