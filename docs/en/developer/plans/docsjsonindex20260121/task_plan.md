# Task Plan: Auto-update docs.json for plan sessions
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. docsjsonindex20260121 */}
...
Keep `docs/docs.json` automatically in sync with `docs/en/developer/plans/<session-hash>/{task_plan,findings,progress}.md`, and backfill missing navigation entries for existing sessions.
...
1. What exact Mintlify page IDs should be added for each session file (task_plan/findings/progress)?
2. How should we handle incomplete session folders (missing one of the three files) to avoid broken links?
3. Should `init-session.sh` do an incremental update (new session only) or a deterministic full sync?
