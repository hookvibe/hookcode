# Task Plan: Task group worker environment handling
<!-- Track phase updates for task-group worker environment handling. docs/en/developer/plans/taskgroup-worker-env-20260203/task_plan.md taskgroup-worker-env-20260203 -->
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. taskgroup-worker-env-20260203 */}
...
Update task-group execution flow so workers skip repo pull/install for non-first tasks on the same worker, and warn users when logs exist without a local task group by indicating a new environment will be used.
...
1. Where is the task-group execution flow implemented (worker orchestration vs task runner)? → `backend/src/agent/agent.ts`
2. What user-facing channel should display the "new environment" warning (log output, UI status, or API response)? → task logs via appendLog

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Use task logs (appendLog) to surface environment-change warnings | Fits existing task-group log stream and is user-visible without API changes |
| Skip dependency installs only when workspace is reused and prior tasks exist | Matches "same worker, not first task" requirement without impacting first runs |
