# Task Plan: 任务组页面日志样式与滚动修复
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. xyaw6rrnebdb2uyuuv4a */}
...
修复任务组页面的执行日志展示：日志区域不再使用卡片背景，同时解决滚动默认不在底部与向下滑动回弹导致的交互/布局问题。
...
1. 任务组页面的“执行日志”由哪个前端组件渲染（以及是否复用通用 LogViewer/Console 组件）？
2. 滚动容器的层级结构与 CSS（overflow/height）是否导致 iOS/macOS 的 rubber-band 回弹或滚动穿透？
3. 当前“自动滚动到底部”逻辑如何实现（是否在渲染后 scrollTop=scrollHeight，或使用虚拟列表），以及如何在用户上滑查看历史时避免强制跳底？

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Scroll the nearest scroll container to bottom in `TaskLogViewer` (flat variant). | Avoid scroll bouncing between multiple log sections and keep the TaskGroup page pinned to the latest content. |
| Make TaskGroup log Card background transparent. | Remove the unwanted card surface behind execution logs. |
| Add `overscroll-behavior: contain` to the chat scroller. | Reduce scroll chaining/rubber-band bounce in the TaskGroup page. |
