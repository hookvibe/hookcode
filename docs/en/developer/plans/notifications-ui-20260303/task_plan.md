...
Improve notification UX so opening the popover marks messages read, "mark all read" closes the popover, settings top nav shows a notification icon (except the notifications tab), and notifications/logs tables use pagination with tests.
...
- [x] Inspect current notification popover and settings page UI
- **Status:** complete
...
1. Where is the notifications popover rendered, and how is read state managed today?
2. How should pagination be implemented with cursor-based APIs in settings tables?
3. What pages lack the notification icon in top navigation, and what should be excluded?

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Add auto-read on popover open and close-on-mark-read | Matches requested UX and keeps read state consistent |
| Replace \"Load more\" with table pagination backed by cursor fetches | Delivers paginated UI while reusing existing cursor APIs |
| Render settings top-nav notification icon except on notifications tab | Ensures consistent access without duplicating the notifications view |
...
| Frontend test failed: popover close assertion in notificationsPopover.test.tsx | 1 | Added popover destroy-on-hide to remove content on close |
| Test warnings: worker process exit warning + antd destroyTooltipOnHide deprecation warnings | 1 | Logged for visibility; tests still passed |
