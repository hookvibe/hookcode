# Task Plan: Update hookcode.yml for preview ports
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. hookcode-yml-update */}
...
Update `.hookcode.yml` so dependency installation and frontend/backend preview commands run on system-assigned ports, while preview env mirrors non-default values from `frontend/.env` and `backend/.env` (including the remote DB settings).
...
1. Which preview instances are required to cover both frontend and backend dev servers?
2. How should the preview environment ensure backend uses the system-assigned `PORT` instead of the fixed `.env` value?
3. How should `VITE_API_BASE_URL` be represented without hardcoding ports?

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Add a backend preview instance with `PORT={{PORT}}` in env. | Ensures Nest picks up HookCode's assigned port even when `backend/.env` defines a fixed `PORT`. |
| Keep frontend preview command using `--port {{PORT}}`. | Vite honors the CLI flag reliably for dynamic ports. |
| Mirror only non-default `.env` values into preview env blocks. | Satisfies the requirement while avoiding redundant defaults. |
| Avoid fixed-port values in preview env entries. | Preview validation rejects fixed ports; placeholders or relative paths are required. |
| Keep DB URL/DB_PORT in `backend/.env` rather than `.hookcode.yml`. | Preview env validation blocks fixed-port DB values but dotenv still loads them. |
