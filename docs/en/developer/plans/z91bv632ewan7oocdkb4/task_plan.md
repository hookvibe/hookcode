...
- [x] Decide sidebar split strategy (multi sidebars vs multi docs instances)
- [x] Define navbar UX and default landing pages
- **Status:** complete
...
1. Do we want separate routeBasePaths (e.g. `/api`, `/developer`) or keep everything under `/docs/*`? → Keep everything under `/docs/*`.
2. Which document should be the default landing page for each top-level navbar entry? → Use each section's `index.md` (first doc in its sidebar).
