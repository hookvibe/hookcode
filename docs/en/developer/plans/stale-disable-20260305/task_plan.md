# Task Plan: Disable stale processing when PROCESSING_STALE_MS is blank
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. stale-disable-20260305 */}
...
Allow PROCESSING_STALE_MS to be blank or zero to disable stale task recovery without auto-failing tasks, while keeping retry/stale tracking consistent and audited.
...
1. Should blank/0 be interpreted as "disable stale recovery" while undefined keeps the default?
2. Which runtime paths (bootstrap, worker, task retry, queue stats) must share the same stale parsing logic?
3. What system log entry should be emitted when stale recovery is disabled?

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|---|---|
| Interpret blank or `0` `PROCESSING_STALE_MS` as "disable stale recovery", while undefined keeps the default timeout. | Matches user intent while keeping safe defaults for environments that omit the variable. |
| Skip stale reaper timers when disabled and log a system audit entry. | Avoids false failures and provides audit visibility for configuration changes. |

## Errors Encountered
{/* WHAT: Every error you encounter, what attempt number it was, and how you resolved it. WHY: Logging errors prevents repeating the same mistakes. This is critical for learning. WHEN: Add immediately when an error occurs, even if you fix it quickly. EXAMPLE: | FileNotFoundError | 1 | Check if file exists, create empty list if not | | JSONDecodeError | 2 | Handle empty file case explicitly | */}
| Error | Attempt | Resolution |
|-------|---------|------------|
| docs.json missing navigation.languages[] | 1 | Non-blocking init-session warning; continue without docs.json sync. |
