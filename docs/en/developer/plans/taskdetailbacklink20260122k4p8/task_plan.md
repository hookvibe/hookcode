# Task Plan: Add HookCode task detail link to provider comments
{/* WHAT: This is your roadmap for the entire task. Think of it as your "working memory on disk." WHY: After 50+ tool calls, your original goals can get forgotten. This file keeps them fresh. WHEN: Create this FIRST, before starting any work. Update after each phase completes. */}

{/* Track code changes with this session hash for traceability. taskdetailbacklink20260122k4p8 */}
...
Append a compact badge-like HookCode task link footer pointing to the current task detail page in every provider-side posted message (GitHub/GitLab), with a configurable public base URL.
...
- [x] Extract shared `getTaskConsoleUrl()` util
- [x] Append `[HookCode in](<task url>)` to provider posts
- **Status:** complete
...
- [x] Add unit tests and run backend test suite
- [x] Run backend build (tsc) for sanity
- **Status:** complete
...
1. What is the canonical frontend route for task detail pages (path format) in this repo?
2. Which env/config value should be used as the public console base URL, and do we need a new `.env.*.example` entry?
3. Should the "HookCode in" line be appended always or only for webhook-triggered tasks?
4. What is the safest fallback behavior when the base URL is not configured?

## Decisions Made
{/* WHAT: Technical and design decisions you've made, with the reasoning behind them. WHY: You'll forget why you made choices. This table helps you remember and justify decisions. WHEN: Update whenever you make a significant choice (technology, approach, structure). EXAMPLE: | Use JSON for storage | Simple, human-readable, built-in Python support | */}
| Decision | Rationale |
|----------|-----------|
| Extract `getTaskConsoleUrl()` into `backend/src/utils/taskConsoleUrl.ts` | Avoid duplicating console URL env parsing across Agent/Reporter and keep provider posts consistent. |
| Append a final footer line `[HookCode in](<task url>)` | Gives a compact, clickable marker back to HookCode task detail in every provider message. |
