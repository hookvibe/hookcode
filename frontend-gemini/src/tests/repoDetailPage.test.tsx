...
describe('RepoDetailPage (frontend-chat migration)', () => {
  ...
  test('saves basic info via updateRepo', async () => {
    ...
    // legacy UI buttons with icons include the icon name in the accessible name (e.g. "save Save").
    const basicCard = nameInput.closest('.hc-ui-card') ?? document.body;
  ...
  test('shows dependency override controls in robot editor', async () => {
    ...
    const failureModeItem = failureModeLabel.closest('.hc-ui-form-item');
    expect(failureModeItem).toBeTruthy();
    const failureModeSelect = failureModeItem?.querySelector('.hc-ui-select');
    expect(failureModeSelect).toHaveClass('hc-ui-select-disabled');

    const overrideLabel = screen.getByText('Override dependency behavior');
    const overrideItem = overrideLabel.closest('.hc-ui-form-item');
    ...
    const failureModeSelectAfter = failureModeItem?.querySelector('.hc-ui-select');
    expect(failureModeSelectAfter).not.toHaveClass('hc-ui-select-disabled');
  });
});