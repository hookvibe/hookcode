...
describe('TaskGroupChatPage (frontend-chat migration)', () => {
  ...
  test('disables sender controls for unsupported task group kinds', async () => {
    ...
    // legacy UI Select uses nested inputs; assert on the closest Select wrapper for the disabled state.
    expect(repoSelect.closest('.hc-ui-select')).toHaveClass('hc-ui-select-disabled');
    expect(robotSelect.closest('.hc-ui-select')).toHaveClass('hc-ui-select-disabled');
