...
import { App, Button, Card, Empty, Input, Skeleton, Space, Tag, Tooltip, Typography } from '@/ui';
import { PlayCircleOutlined, ReloadOutlined, SearchOutlined } from '@/ui/icons';
 ...
 * - 2026-01-11: Replace deprecated legacy UI `List` with a card list to avoid runtime warnings and match the chat-first visual style.
 */
...
export const TasksPage: FC<TasksPageProps> = ({ status, repoId, userPanel }) => {
  ...
  useEffect(() => {
    // Prevent duplicate list fetches in strict-mode-like double effects while keeping filter changes responsive. docs/en/developer/plans/frontendgemini-migration-20260127/task_plan.md frontendgemini-migration-20260127
    const fetchKey = `${statusFilter}|${repoIdFilter ?? ''}`;
    if (initialFetchKeyRef.current === fetchKey) return;
    initialFetchKeyRef.current = fetchKey;
    void refresh();
  }, [refresh, repoIdFilter, statusFilter]);
