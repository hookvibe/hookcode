...
if [ ! -f "$PLAN_FILE" ]; then
    ...
    echo "Usage:"
    echo "  bash .gemini/skills/file-context-planning/scripts/check-complete.sh <plan-path|session-hash>"
    echo ""
    echo "Examples:"
    echo "  bash .gemini/skills/file-context-planning/scripts/check-complete.sh docs/en/developer/plans/<hash>/task_plan.md"
    echo "  bash .gemini/skills/file-context-planning/scripts/check-complete.sh <hash>"
    exit 1
fi
