...
#
# Usage:
#   bash .gemini/skills/file-context-planning/scripts/append-changelog.sh "<session-hash>" "<one-line-summary>"
...
if [ -z "${SESSION_HASH}" ]; then
    ...
    echo "Usage:"
    echo "  bash .gemini/skills/file-context-planning/scripts/append-changelog.sh \"<session-hash>\" \"<one-line-summary>\""
    echo "  printf '%s' \"<one-line-summary>\" | bash .gemini/skills/file-context-planning/scripts/append-changelog.sh \"<session-hash>\""
    exit 1
fi
