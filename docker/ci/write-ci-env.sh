...
if [[ "${work_dir_root}" != /* ]]; then
  echo "[ci] ERROR: HOOKCODE_WORK_DIR must be an absolute container path (example: /var/lib/hookcode)" >&2
  exit 1
fi
...
write_kv TASK_LOGS_VISIBLE_ENABLED "${TASK_LOGS_VISIBLE_ENABLED:-true}"
